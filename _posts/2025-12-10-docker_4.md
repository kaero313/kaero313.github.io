...
이제 빌드 성공 유무에 따른 도커 허브 업로드 자동화까지 적용하였다. 처음 목표는 여기까지였지만 현재 상태에서는 빌드 및 업로드 이후 서버 배포는 수동으로 해야 하는
상황이기 때문에, 좀 더 자동화의 의미에 맞으려면 서버 배포 자동화까지 구축하는게 맞다고 생각하여 해당 항목까지 진행해보도록 하겠다.
